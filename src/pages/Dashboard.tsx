...
> = {
  likely_real: {
    ...
    description:
      "This content appears to be based on credible sourcing and journalistic standards.",
  },
  uncertain: {
    label: "Uncertain",
    icon: AlertTriangle,
    ...
    description:
      "This content has a mix of credible and questionable elements. Exercise caution.",
  },
  likely_fake: {
    label: "Likely Fake",
    icon: XCircle,
    ...
    description:
      "This content shows multiple indicators of misinformation or manipulation.",
  },
};
          ...
          <div className="flex items-center gap-0.5 sm:gap-1">
            {navItems.map((item) => (
              <Button
                ...
                size="sm"
                className={`cursor-pointer gap-1 sm:gap-1.5 text-[11px] sm:text-xs px-2 sm:px-3 h-8 ${
                  activeView === item.key
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground">
                      Paste any news article, social media post, or content to verify
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="glass-card rounded-2xl p-1 mb-4">
                <Textarea
                  ...
                  placeholder={
                    inputType === "text"
                      ? "Paste a news article, social media post, or any text content here..."
              ...
              <Button
                size="lg"
                className="cursor-pointer w-full bg-primary hover:bg-primary/90 text-primary-foreground gap-2 h-12 sm:h-13 text-sm sm:text-base glow-blue"
                ...
                <div className="flex flex-col sm:flex-row items-center gap-6">
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    <div className="flex items-center gap-3 mb-2 justify-center sm:justify-start">
                      <div
                        className={`w-10 h-10 rounded-xl ${vc.bg} flex items-center justify-center`}
                      >
                        <vc.icon className={`w-5 h-5 ${vc.color}`} />
                      </div>
                      <h2 className={`text-xl sm:text-2xl font-bold ${vc.color}`}>
                        {vc.label}
                      </h2>
                    </div>
                      ...
                      {currentResult.redFlags.length > 0 && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-red-600 font-medium">
                            {currentResult.redFlags.length} red flag{currentResult.redFlags.length !== 1 ? "s" : ""}
                          </span>
                        </div>
                      )}
                      {currentResult.greenFlags.length > 0 && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-emerald-600 font-medium">
                            {currentResult.greenFlags.length} green flag{currentResult.greenFlags.length !== 1 ? "s" : ""}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                ...
                >
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    {currentResult.redFlags.length > 0 && (
                      <Badge variant="outline" className="border-red-500/30 text-red-600 text-[10px] ml-auto">
                        {currentResult.redFlags.length}
                      </Badge>
                    )}
                  </div>
                  {currentResult.redFlags.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic">No red flags detected</p>
                  ) : (
                ...
                >
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    {currentResult.greenFlags.length > 0 && (
                      <Badge variant="outline" className="border-emerald-500/30 text-emerald-600 text-[10px] ml-auto">
                        {currentResult.greenFlags.length}
                      </Badge>
                    )}
                  </div>
                  {currentResult.greenFlags.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic">No green flags detected</p>
                  ) : (
                    <ul className="space-y-2">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3 sm:gap-4">
                          <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl ${avc.bg} flex items-center justify-center shrink-0`}>
                            <avc.icon className={`w-4 h-4 sm:w-5 sm:h-5 ${avc.color}`} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className={`text-xs sm:text-sm font-semibold ${avc.color}`}>
                                {avc.label}
                              </span>
                              <Badge variant="outline" className={`text-[10px] ${avc.border} ${avc.color}`}>
                                {analysis.confidence}%
                              </Badge>
                              <Badge variant="outline" className="text-[10px] ml-auto">
                                {analysis.inputType === "url" ? <Link className="w-3 h-3 mr-1" /> : <FileText className="w-3 h-3 mr-1" />}
            ...
            >
              <div className="mb-6 sm:mb-8">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground">
                      Overview of your analysis activity
                    </p>
                  </div>
                </div>
              </div>
              ...
              {!analyses ? (
                ...
                <div className="glass-card rounded-2xl p-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-6">
                    Analyze some content to see your statistics.
                  </p>
                  <Button
                    className="cursor-pointer bg-primary text-primary-foreground gap-2"
                    onClick={() => setActiveView("analyze")}
              ...
              <div className="mb-6 sm:mb-8">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground">
                      Technical documentation of the detection approach
                    </p>
                  </div>
                </div>
              </div>
