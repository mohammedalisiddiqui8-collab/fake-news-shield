...
export default function GlyphPortal({
  ...
  return (
    <section ref={sectionRef} id={uid} className={className} aria-label={text}
      style={{ "--gp-length": String(length), "--gp-characters": String(Array.from(text).length), ...style } as CSSProperties}>
      <style>{`
        ...
        ${q} [data-gp-letter]{box-sizing:border-box;position:absolute;border:0;padding:0;margin:0;background:transparent;cursor:pointer;pointer-events:auto;}
        ${q}[data-gp-motion=off] [data-gp-letter]{display:none;}
        ${q} [data-gp-glyph]{fill:var(--gp-ink);opacity:0;transition:opacity .3s ease;}
        ${q}[data-gp-ready][data-gp-motion=off] [data-gp-glyph]{opacity:1;}
        ${q} [data-gp-select]{position:absolute;top:0;left:0;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0;padding:0;margin:0;}
        ${q} [data-gp-caption]{position:absolute;inset:0;display:flex;flex-direction:column;justify-content:flex-end;padding:clamp(16px,4cqi,40px);opacity:var(--gp-caption,1);pointer-events:var(--gp-caption-hit,auto);z-index:1;}
        ${q} [data-gp-caption]>p{margin:0;font-size:clamp(12px,1.6cqi,18px);line-height:1.5;color:var(--gp-foreground);opacity:.7;max-width:42ch;}
        ${q} [data-gp-caption]>h2{margin:0 0 .5em;font-size:clamp(20px,3cqi,32px);font-weight:700;line-height:1.2;color:var(--gp-foreground);}
        ${q} [data-gp-foreground]{position:absolute;inset:0;opacity:var(--gp-reveal,0);pointer-events:none;z-index:2;}
        ${q}[data-gp-entered=true] [data-gp-foreground]{opacity:1;}
        ${q} [data-gp-annotations]{stroke:var(--gp-ink);fill:none;opacity:.4;stroke-width:1;}
      `}</style>
      <div data-gp-viewport aria-hidden="true" />
      <div data-gp-pin>
        {background && <div data-gp-field>{background}</div>}
        <svg data-gp-art aria-hidden="true">
          <defs>
            <clipPath id={clipId} clipPathUnits="userSpaceOnUse">
              <text data-gp-glyph textAnchor="middle" dominantBaseline="central"
                transform="translate(400 138)">
                {characters.map(({ char, index }) => <tspan key={index}>{char}</tspan>)}
              </text>
            </clipPath>
          </defs>
          {annotations && <g data-gp-marks><path /></g>}
        </svg>
        {children && <div data-gp-caption>{children}</div>}
        {hasFront && <div data-gp-foreground>{front}</div>}
      </div>
      <div data-gp-choices role="radiogroup" aria-label={`${text} letter selection`}>
        {characters.map(({ char, index }) => (
          <button key={index} data-gp-letter={index} role="radio" aria-label={char}
            aria-checked="false" tabIndex={-1} disabled>{char}</button>
        ))}
      </div>
      <select data-gp-select aria-label={`${text} letter picker`}>
        <option value="">{enterLabel}</option>
        {characters.map(({ char, index }) => (
